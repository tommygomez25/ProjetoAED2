...
vector<string> filesVector(){
    ...
    DIR *pDIR;
    struct dirent *entry;
    pDIR = opendir("D:\\MIEIC-LEIC\\2_ano\\AED\\ProjetoAED2\\dataset");
...
/*
    ...
    cout << endl << graph1.bfs(1262, 1339);
    cout << endl << graph1.dijkstra_distance(1262,1339) << endl;
    list<int> path = graph1.dijkstra_path(1262,1339);
    ...
    cout << endl << graph1.dijkstra_distance(1373,1216) << endl;
    path = graph1.dijkstra_path(1373,1226);
    for(auto it = path.begin(); it != path.end(); it++){
        cout << (*it) << ' ';
    }
    cout << endl;
